...
#include <stdio.h>
#include <stdlib.h>
#include <stdbool.h>//使支持布尔型 

typedef struct{//通过typedef结构体定义node（节点）数据类型 
	int data;//节点的数据域 
	struct node* next;//节点的指针域，为指向下一个节点的指针 
}node;

node* createLink(){//创建一个空链表，只包含控制节点（入口节点、起始节点、终末节点），不包含数据节点 
	node* entry= (node*)malloc(sizeof(node));//创建入口节点 
	node* head = (node*)malloc(sizeof(node));//创建起始节点 
	node* tail = (node*)malloc(sizeof(node));//创建终末节点 
...
在本程序中，起始节点的序号为0，数据节点的序号为正整数 */ 
	...
	node* insert = (node*)malloc(sizeof(node));//创建一个新的数据节点 
